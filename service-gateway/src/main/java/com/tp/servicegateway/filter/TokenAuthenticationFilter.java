package com.tp.servicegateway.filter;
 ...
 *
 * 在网关项目中添加GatewayFilter过滤器
 * 和全局过滤器u同的是，GatewayFilter需要在配置文件中指定哪个服务使用此过滤器
 */
@Component
public class TokenAuthenticationFilter extends AbstractGatewayFilterFactory {

    private static final String Bearer_ = "Bearer_";
        ...
        return (exchange, chain) -> {
            ...
            try {
                ...
                if (!StringUtils.isEmpty(token)) {
                    System.out.println("TokenAuthenticationFilter>>>> 验证通过");
                    //3.有token，把token设置到header中，传递给后端服务
                    mutate.header("userDetails", token).build();
                } else {
                    //4.token无效
                    System.out.println("TokenAuthenticationFilter>>>> token无效");
